...
namespace Render
{
    ...
    void TextureResource::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
    {
        ...
        if (vkCreateImage(pipeLine->vkInstance->vDevice, &imageInfo, nullptr, &image) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create image!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(pipeLine->vkInstance->vDevice, image, &memRequirements);

        VkMemoryAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocInfo.allocationSize = memRequirements.size;
        allocInfo.memoryTypeIndex = pipeLine->vkInstance->findMemoryType(memRequirements.memoryTypeBits, properties);

        if (vkAllocateMemory(pipeLine->vkInstance->vDevice, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to allocate image memory!");
        }

        vkBindImageMemory(pipeLine->vkInstance->vDevice, image, imageMemory, 0);
    }
    void TextureResource::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout newLayout, VkImageLayout oldLayout)
    {
    ...
    {
        textureImageView = pipeLine->vkInstance->CreateImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB);
    }
