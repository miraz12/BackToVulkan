...
namespace Render
{
 ...
 texture->mipLevels, VK_SAMPLE_COUNT_1_BIT, texture->image, texture->deviceMemory);
	...
	void MeshResource::LoadNode(Node* parent, tinygltf::Node& node, uint32_t nodeIndex, tinygltf::Model& model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
	{
		...
		if (node.mesh > -1) 
		{
			...
			Mesh* newMesh = new Mesh(pipeline->vkInstance->vDevice);
			...
			for (size_t i = 0; i < mesh.primitives.size(); i++)
			{
				...
				float* bufferTexCoordSet0 = nullptr;
				float* bufferTexCoordSet1 = nullptr;
				...
				if (primitive.attributes.find("NORMAL") != primitive.attributes.end()) 
				{
					...
					bufferNormals = reinterpret_cast<float*>(&(model.buffers[normView.buffer].data[normAccessor.byteOffset + normView.byteOffset]));
				...
				if (primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end()) 
				{
					...
					bufferTexCoordSet0 = reinterpret_cast<float*>(&(model.buffers[uvView.buffer].data[uvAccessor.byteOffset + uvView.byteOffset]));
				...
				if (primitive.attributes.find("TEXCOORD_1") != primitive.attributes.end()) 
				{
					...
					const tinygltf::BufferView& uvView = model.bufferViews[uvAccessor.bufferView];
					bufferTexCoordSet1 = reinterpret_cast<float*>(&(model.buffers[uvView.buffer].data[uvAccessor.byteOffset + uvView.byteOffset]));
					uv1ByteStride = uvAccessor.ByteStride(uvView) ? (uvAccessor.ByteStride(uvView) / sizeof(float)) : tinygltf::GetNumComponentsInType(TINYGLTF_TYPE_VEC2);
				}
				for (size_t v = 0; v < posAccessor.count; v++) 
				{
					Vertex vert{};
					vert.pos = Math::vector3D(*reinterpret_cast<Math::vector3D*>(&bufferPos[v * posByteStride]));
					vert.normal = ((Math::vector3D(bufferNormals ? Math::vector3D(*reinterpret_cast<Math::vector3D*>(&bufferNormals[v * normByteStride])) : Math::vector3D(0.0f)))).normalizeRe();
					vert.UV = bufferTexCoordSet0 ? Math::vector2D(*reinterpret_cast<Math::vector2D*>(&bufferTexCoordSet0[v * uv0ByteStride])) : 0.0f;
