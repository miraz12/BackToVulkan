...
namespace Render
{
	...
	void MeshResource::LoadModel(std::string filename, GraphicsPipeline* pipe, float scale)
	{
		...
		} vertexStaging{ 0 }, indexStaging{0};

		// Create staging buffers
		// Vertex data
		pipeline->CreateBuffer(vertexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vertexStaging.buffer, vertexStaging.memory);
		void* data;
		vkMapMemory(pipeline->vkInstance->vDevice, vertexStaging.memory, 0, vertexBufferSize, 0, &data);
		memcpy(data, vertexBuffer.data(), (size_t)vertexBufferSize);
		vkUnmapMemory(pipeline->vkInstance->vDevice, vertexStaging.memory);

		// Index data
		if (indexBufferSize > 0) 
		{
			pipeline->CreateBuffer(indexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, indexStaging.buffer, indexStaging.memory);
			vkMapMemory(pipeline->vkInstance->vDevice, indexStaging.memory, 0, indexBufferSize, 0, &data);
			memcpy(data, indexBuffer.data(), (size_t)indexBufferSize);
			vkUnmapMemory(pipeline->vkInstance->vDevice, indexStaging.memory);
		}
		...
		//Vertex copy
		pipeline->CopyBuffer(indexStaging.buffer, indices.buffer, indexBufferSize);

		//Index copy
		if (indexBufferSize > 0)
		{
			pipeline->CopyBuffer(vertexStaging.buffer, vertices.buffer, vertexBufferSize);
		}

		//Cleanup staging resources
		vkDestroyBuffer(pipeline->vkInstance->vDevice, vertexStaging.buffer, nullptr);
		vkFreeMemory(pipeline->vkInstance->vDevice, vertexStaging.memory, nullptr);
		vkDestroyBuffer(pipeline->vkInstance->vDevice, indexStaging.buffer, nullptr);
		vkFreeMemory(pipeline->vkInstance->vDevice, indexStaging.memory, nullptr);
	}
	...
	void MeshResource::LoadNode(Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
	{
		...
		newNode->matrix = Math::matrix4D(1.0f);
		...
		if (node.mesh > -1) 
		{
			...
			for (size_t i = 0; i < mesh.primitives.size(); i++)
			{
				...
				{
					Vertex vert{};
					vert.pos = Math::vector3D(Math::vector3D(bufferPos[v * posByteStride]));
					vert.normal = ((Math::vector3D(bufferNormals ? Math::vector3D(bufferNormals[v * normByteStride]) : Math::vector3D(0.0f)))).normalizeRe();
					vert.uv0 = *(bufferTexCoordSet0 ? Math::vector2D(bufferTexCoordSet0[v * uv0ByteStride]) : 0.0f);
					vert.uv1 = *(bufferTexCoordSet1 ? Math::vector2D(bufferTexCoordSet1[v * uv1ByteStride]) : 0.0f);
	...
	{
		//vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &node->mesh.uniformBuffer.descriptorSet, 0, nullptr);
