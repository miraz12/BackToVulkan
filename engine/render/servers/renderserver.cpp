...
namespace Render
{
	RenderServer::RenderServer(Core::VulkanInstance* vkInstance)
	{
...
#pragma warning (pop)
	...
	void RenderServer::RecreateSwapChain()
	{
		CleanupSwapChain();
		CreateRenderPass();
		CreateGraphicsPipeline();
	...
	void RenderServer::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
	...
	VkCommandBuffer RenderServer::BeginSingleTimeCommands()
	{
	...
	void RenderServer::EndSingleTimeCommands(VkCommandBuffer commandBuffer)
	{
	...
	void RenderServer::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
	{
	...
	void RenderServer::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) 
	{
		VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
	...
	void RenderServer::CleanupSwapChain()
	{
		vkDestroyImageView(vkInstance->vDevice, colorImageView, nullptr);
		vkDestroyImage(vkInstance->vDevice, colorImage, nullptr);
		vkFreeMemory(vkInstance->vDevice, colorImageMemory, nullptr);
	...
	void RenderServer::CreateUniformBuffers()
	{
	...
	void RenderServer::UpdateUniforms(uint32_t currentImage)
	{
	...
	void RenderServer::CreateDescriptorPool()
	{
	...
	void RenderServer::CreateDescriptorSets()
	{
	...
	void RenderServer::CreateDepthResources()
	{
		VkFormat depthFormat = FindDepthFormat();
		CreateImage(vkInstance->swapChain.swapChainExtent.width,
					vkInstance->swapChain.swapChainExtent.height,
					depthFormat, VK_IMAGE_TILING_OPTIMAL,
					VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 
					VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 1, vkInstance->msaaSamples, depthImage, depthImageMemory);

		depthImageView = vkInstance->CreateImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);

		TransitionImageLayout(depthImage, depthFormat, 1, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
	}

	void RenderServer::CreateColorResources()
	{
		VkFormat colorFormat = vkInstance->swapChain.swapChainImageFormat;

		CreateImage(vkInstance->swapChain.swapChainExtent.width, vkInstance->swapChain.swapChainExtent.height,
					colorFormat, VK_IMAGE_TILING_OPTIMAL, 
					VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
					VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 1, vkInstance->msaaSamples, colorImage, colorImageMemory);

		colorImageView = vkInstance->CreateImageView(colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}

	void RenderServer::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkImage& image, VkDeviceMemory& imageMemory)
	{
	...
	void RenderServer::TransitionImageLayout(VkImage image, VkFormat format, uint32_t mipLevels, VkImageLayout newLayout, VkImageLayout oldLayout)
	{
		VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
	...
	void RenderServer::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
	{
		// Check if image format supports linear blitting
		VkFormatProperties formatProperties;
		vkGetPhysicalDeviceFormatProperties(vkInstance->physicalDevice, imageFormat, &formatProperties);
	...
	void RenderServer::CreateDescriptorSetLayout()
	{
		//Uniform buffers
	...
	void RenderServer::CreateGraphicsPipeline()
	{
		...
		multisampling.rasterizationSamples = vkInstance->msaaSamples;
	...
	void RenderServer::CreateRenderPass()
	{
		VkAttachmentDescription colorAttachment{};
		colorAttachment.format = vkInstance->swapChain.swapChainImageFormat;
		colorAttachment.samples = vkInstance->msaaSamples;
		...
		depthAttachment.samples = vkInstance->msaaSamples;
		...
		colorAttachmentResolve.format = vkInstance->swapChain.swapChainImageFormat;
	...
	void RenderServer::CreateFramebuffers()
	{
	...
	{
		Core::QueueFamilyIndices queueFamilyIndices = vkInstance->FindQueueFamilies(vkInstance->physicalDevice);
	...
	void RenderServer::CreateCommandBuffers()
	{
